...
Form.addEventListener('submit', function (event) {
    ...
    let resultText = `Monthly Payment: $${PaymentMonthly.toFixed(2)}<br>
                    Total Payment: $${TotalPayment.toFixed(2)}<br>
                    Total Interest: $${InterestTotal.toFixed(2)}`;
...
function calculate() {
    ...
    BuyRentResult.innerHTML = `Total Cost of Buying: $${TotalCostBuy.toFixed(2)}<br>Total Cost of rent: $${TotalCostRent.toFixed(2)}<br>${TotalCostBuy > TotalCostRent ? "You should rent." : "You should buy."}`;
}